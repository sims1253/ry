...
| RY010 | unbound-variable         | warning  | Reference to a variable with no binding in scope. Not reported inside a closure whose formals or body are installed dynamically via `formals(f) <- ...` / `body(f) <- ...`: the construction site, not the placeholder literal, decides what the closure binds.                                   |
...
| RY080 | map-return-type-mismatch | error    | A purrr typed-map (`map_dbl`, `map_int`, ...) callback returns a value whose mode is incompatible with the target vector type. R rejects incompatible callback results. A callback built through a `body(f) <- ...` replacement is not judged by the placeholder literal's returns.          |
