...
| RY110 | vacuous-all-guard | warning | `all(is.na(x))` is vacuously TRUE for zero-length `x`, so a validation guard combining a type predicate with that operand — `is.numeric(x) or all(is.na(x))` — admits empty input failing the predicate, which a downstream stub-declared mode demand then rejects. Guard the emptiness too, as hms did: `length(x) > 0 &&` before the `all()`. |
