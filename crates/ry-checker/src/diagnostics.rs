...
/// Amend the body of an existing comment (the text after `#`) so its
/// suppression directive also covers `code`. Lets quick fixes merge into
/// a directive that already sits on the line instead of appending a
/// second `# ry: ignore[...]` after it — which the parser would ignore,
/// because everything after the first `#` is one comment and the
/// directive must start that comment's body (#210).
///
/// Returns `None` when the body is not a recognized line directive
/// (prose, or a `ry: ignore-file` marker); the caller should place a
/// fresh directive at the start of the comment instead. Returns the
/// body unchanged when the directive already covers `code` — including
/// bare directives, whose empty rule list suppresses everything.
///
/// The rule list is rewritten in bracket form (`ry: ignore[RY040,
/// RY010]`), preserving the original marker spelling and any prose that
/// follows the rule list.
pub fn amend_ignore_comment_body(body: &str, code: &str) -> Option<String> {
    let trimmed = body.trim_start();
    let lower = trimmed.to_lowercase();
    let marker_len = ["ry: ignore", "ry:ignore", "noqa"]
        .into_iter()
        .find(|marker| lower.starts_with(marker))
        .map(|marker| marker.len())?;
    let after = &trimmed[marker_len..];
    // `ry: ignore-file` is a file-level directive, never a line one.
    if marker_len > "noqa".len() && after.trim_start().starts_with("-file") {
        return None;
    }

    let mut codes = parse_rule_codes(after);
    let code = code.trim().to_uppercase();
    if codes.is_empty() || codes.iter().any(|existing| existing == &code) {
        return Some(body.to_string());
    }
    codes.push(code);

    // Preserve the whitespace between `#` and the marker, the marker's
    // original spelling, and any prose after the rule list. The rule
    // list itself is replaced with the merged bracket form.
    let lead = &body[..body.len() - trimmed.len()];
    let after_trim = after.trim_start();
    // Back off the whitespace separating the rule list from the prose so
    // the rewritten bracket list does not glue itself to the first word.
    let rule_region_end = rule_list_end(after_trim);
    let rule_region_end = after_trim[..rule_region_end].trim_end().len();
    let prose = &after_trim[rule_region_end..];
    Some(format!(
        "{}{}[{}]{}",
        lead,
        &trimmed[..marker_len],
...
/// Byte length of the leading rule-list region of a directive body —
/// `[RY040]` (through the closing bracket, or the whole rest when the
/// bracket is unterminated) or the `: RY040, RY010` token run. Anything
/// from the first token that is neither a code nor a list separator
/// (`,`, `:`, brackets) onward is prose and is left in place.
fn rule_list_end(text: &str) -> usize {
    ...
    for token in text.split_inclusive([',', ' ']) {
        ...
        if word.is_empty() || word == ":" || word == "[" || word == "]" || is_code_token(word) {
            end += token.len();
        } else {
            break;
        }
...
/// Whether a whitespace-delimited token looks like a rule code (`RY040`,
/// `ry010`): alphanumeric with at least one digit. Prose words
/// (`note`, `TODO`) fail this, which is what separates a colon-style
/// rule list from the prose behind it.
