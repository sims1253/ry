...
6. Before the 0.9.0 Zed release, complete
   [binary integrity verification (#80)](https://github.com/sims1253/ry/issues/80).
   The extension currently downloads executables without checking their digest.
   The issue requires executable `.bin.sha256` sidecars in the core release
   and tests that reject missing, malformed, or mismatched sidecars. Existing
   archive `.sha256` files cannot verify an extracted executable. Keep
   `release.yml` generated by cargo-dist; add release steps through a supported
   hook or a separate workflow.
