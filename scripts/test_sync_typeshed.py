...
""")
    ...
    def test_sync_preserves_the_local_annotation_overlay(self):
        # ry-side stub annotations (crates/ry-typeshed/overlay) must
        # survive a vendor sync: the sync wholesale-replaces vendor/ from
        # the r-typeshed checkout -- whose stubs carry no ry-only specs --
        # while the overlay directory sits outside vendor/ and stays put.
        # ry merges vendor + overlay at typeshed load time, so the
        # effective annotations are intact after every weekly bump.
        overlay = self.repo / "crates/ry-typeshed/overlay"
        overlay.mkdir(parents=True)
        overlay_entry = overlay / "base.json"
        overlay_entry.write_text('{"package": "base", "return_mode": "kept"}\n')
        result = self.run_sync(0)
        self.assertEqual(result.returncode, 0, result.stderr)
        # vendor was replaced wholesale from upstream, which has no
        # return_mode annotation anywhere.
        self.assertEqual((self.vendor / "base/base.json").read_bytes(),
                         (self.checkout / "stubs/base/base.json").read_bytes())
        self.assertNotIn("return_mode",
