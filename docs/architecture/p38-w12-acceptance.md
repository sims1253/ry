...
**Later revision.** The foundation (`AnalysisHost`, `AnalysisSnapshot`,
`SymbolIndex`) was later deleted; no consumer ever used it. `ry-analysis`
keeps only `check.rs`, and the LSP handlers keep their open-document scope.
