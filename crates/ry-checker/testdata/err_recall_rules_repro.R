# expect: RY102, RY103, RY105, RY107
...
# which the same package writes correctly at R/glue.R:139. Originally left
# open because the shape seemed indistinguishable from diffobj's legitimate
# `!all(diff(x)) == 1L` (pinned as must-stay-silent in
# ry095_ry096_real_shapes.R). RY107 separates them by outcome: `== 0`
# negates the scalar logical while `== 1` preserves it, so only the negating
# and constant comparisons are reported.
u <- function(x) {
