...
static inline bool scan_close_bracket2(TSLexer* lexer, Scopes* scopes) {
  // The first `]` leaves one bracket scope open. The ordinary close-bracket
  // scanner consumes the second, with whitespace and comments handled normally
  // between tokens. Preserve this state in the existing serialized scope stack.
  if (!scopes_pop(scopes, SCOPE_BRACKET2)) {
    return false;
  }
  return scan_open_block(lexer, scopes, SCOPE_BRACKET, CLOSE_BRACKET2);
}
