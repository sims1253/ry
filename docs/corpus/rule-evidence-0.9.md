...
| `RY032` scalar-logical-length | 1/47 | yes | `unknown_short_circuit_parameter.R` | param-unreachable | piloted | yes | keep | Standing case (P34-W2 policy): 1 TP / 47 FP. Literal-only by design; unknown parameter length is not actionable. R7 confirms parameter-unreachable for c() defaults. |
