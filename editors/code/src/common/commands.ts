...
export function showServerLogsCommand(serverChannel: vscode.OutputChannel): void {
    serverChannel.show();
}
...
export async function explainRuleCommand(binaryPath: string): Promise<void> {
    ...
    try {
        const listOutput = cp.execSync(`"${binaryPath}" explain rules --output-format json`, {
            encoding: "utf-8",
            timeout: 5000,
        });
        const rules = JSON.parse(listOutput) as Array<{ code: string; name: string }>;
